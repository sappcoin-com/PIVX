    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zSAPP Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zSAPP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSAPP seed used to mint this zSAPP is not the same as currently hold in the wallet";
            else
