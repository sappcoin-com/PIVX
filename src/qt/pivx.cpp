...
{
    QMessageBox::critical(0, "Runaway exception", PIVXGUI::tr("A fatal error occurred. SAPP can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("SAPP didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
